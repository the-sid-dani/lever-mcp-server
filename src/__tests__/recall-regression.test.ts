...
describe('recall regression: lever_search_candidates name sweep across pages (BUG-002)', () => {
	it('accumulates matches across 3 pages and reports complete coverage', async () => {
		const client: any = {
			getOpportunities: vi
				.fn()
				.mockResolvedValueOnce({ data: [{ id: 'a', name: 'Jordan Smith' }], hasNext: true, next: 'o2' })
				.mockResolvedValueOnce({ data: [{ id: 'b', name: 'Jordan Smithson' }], hasNext: true, next: 'o3' })
				.mockResolvedValueOnce({ data: [{ id: 'c', name: 'Jordan Smithers' }], hasNext: false }),
		};
