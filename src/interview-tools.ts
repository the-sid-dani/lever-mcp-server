	...
	server.tool(
		...
		"View scheduled or completed interviews for a candidate (opportunity_id) with dashboard, detailed, analytics, or preparation views; note the interviewer_email path is a client-side fan-out (paginate opportunities, fetch each one's interviews, filter by interviewer) since Lever has no server-side interviewer filter, so it is not an exact or fast server-side lookup.",
		{
			...
			owner_email: z
				.string()
				.optional()
				.describe("Get interviews for specific posting owner"),
			...
			// WHEN - Time filtering
			time_scope: z
				.enum(["past_week", "this_week", "next_week", "this_month", "custom"])
				.default("this_week"),
			...
			view_type: z
				.enum(["dashboard", "detailed", "analytics", "preparation"])
				.default("dashboard"),
			...
			status_filter: z
				.enum(["scheduled", "completed", "needs_feedback", "needs_scheduling", "conflicts"])
				.optional(),
			...
			try {
				...
				if (args.opportunity_id) {
					try {
						...
						const interviewsResponse = await client.getOpportunityInterviews(
							args.opportunity_id,
						);
						const interviews = interviewsResponse.data || [];

						// Get panels for additional context
						const panelsResponse = await client.getOpportunityPanels(
							args.opportunity_id,
						);
					...
					const matchedInterviews = allInterviews.filter((iv) => {
						...
						if (interviewerNeedle) {
							const hit = (iv.interviewers || []).some(
								(int) =>
									String(int.email || "").toLowerCase() === interviewerNeedle,
							);
							if (!hit) return false;
						}
						// Time-window match.
	...
	server.tool(
		...
		"Schedule, reschedule, or cancel a candidate interview (creates within a panel); a perform_as user ID is required for any modifying action.",
		{
			...
			opportunity_ids: z
				.array(z.string())
				.optional()
				.describe("Multiple opportunity IDs for bulk operations"),

			// User performing the action (required for create/update/delete)
			perform_as: z
				.string()
				.optional()
				.describe("User ID to perform action as (required for modifications)"),

			// Scheduling data
			interview_details: z
				.object({
					type: z
						.enum(["phone_screen", "technical", "onsite", "panel", "final"])
						.optional(),
					...
					feedback_template: z
						.string()
						.optional()
						.describe("Default feedback template ID"),
				...
				switch (args.action) {
					case "schedule": {
						...
						const panelData = {
							...
							interviews: [
								{
									subject:
										args.interview_details.subject ||
										`${args.interview_details.type} Interview`,
									note: args.interview_details.note,
						...
						result = await client.createPanel(
							args.opportunity_id!,
							panelData,
							performAs,
						);
					...
					case "reschedule": {
						if (!args.interview_id || !args.reschedule_data) {
							throw new Error(
								"interview_id and reschedule_data required for rescheduling",
							);
						}
					...
					case "bulk_schedule":
						if (!args.opportunity_ids || !args.interview_details) {
							throw new Error(
								"opportunity_ids and interview_details required for bulk scheduling",
							);
						}

						result = {
							message:
								"Bulk scheduling would create individual panels for each opportunity",
							...
							text: JSON.stringify(
								{
									...
									hint: error.message.includes("externallyManaged")
										? "This interview was created in Lever UI and cannot be modified via API"
										: undefined,
								},
								null,
								2,
...
function formatAnalyticsView(interviews: LeverInterview[], panels: LeverPanel[]) {
	...
	return {
		...
		panel_usage: {
			...
			average_interviews_per_panel: panels.length > 0 ? interviews.length / panels.length : 0,
		},
	};
}

function formatPreparationView(interviews: LeverInterview[], _panels: LeverPanel[]) {
	...
	return {
		next_interview: upcoming[0]
			? {
					...
					time_until: Math.floor(
						(new Date(upcoming[0].date).getTime() - Date.now()) / 1000 / 60,
					),
