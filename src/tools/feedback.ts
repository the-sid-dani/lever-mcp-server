...
export function registerFeedbackTools(server: McpServer, client: LeverClient) {
	...
	server.tool(
		...
		{
			action: z
				.enum(["list_templates", "list", "get", "submit"])
				.describe(
			...
			opportunity_id: z
				.string()
				.optional()
				.describe("Opportunity / candidate ID. Required for actions: list, get, submit."),
			limit: z
				.number()
				.default(100)
				.optional()
				.describe(
					"For action='list_templates' or 'list' only — max items per request (Lever max 100, paginated up to 5 batches).",
				),
			feedback_id: z
				.string()
				.optional()
				.describe("For action='get' only — feedback form ID to fetch."),
			base_template_id: z
				.string()
				.optional()
				.describe(
					"For action='submit' only — feedback template UID. Use action='list_templates' to discover available templates.",
				),
			field_values: z
				.array(
					z.object({
						...
						value: z
							...
							.describe(
								"Field value. String for most types, array of strings for multiple-select. For score-system fields, use the option text like '3 - Yes' not the option UUID.",
							),
					}),
				)
				.optional()
				.describe(
					"For action='submit' only — array of {id, value} pairs matching the template's required fields. Posted to Lever as `fieldValues[]` (write-shape; asymmetric with GET response `fields[]`).",
				),
			interview_id: z
				.string()
				.optional()
				.describe(
					"For action='submit' only — interview UID to link feedback to (required if panel_id specified).",
				),
			panel_id: z
				.string()
				.optional()
				.describe(
					"For action='submit' only — interview panel UID (required if interview_id specified).",
				),
			mark_complete: z
				.boolean()
				.default(true)
				.optional()
				.describe(
					"For action='submit' only — when true (default), marks feedback as submitted/complete via completedAt timestamp. When false, creates as draft in the Lever UI for human review before finalization. (Lever quirk: omitting completedAt = draft, contradicting their docs.)",
				),
		},
		async (args) => {
			try {
				switch (args.action) {
					...
					case "list": {
						if (!args.opportunity_id)
							throw new Error("opportunity_id is required for action='list'");
					...
					case "get": {
						if (!args.opportunity_id)
							throw new Error("opportunity_id is required for action='get'");
						if (!args.feedback_id)
							throw new Error("feedback_id is required for action='get'");
						const response = await client.getFeedback(
							args.opportunity_id,
							args.feedback_id,
						);
					...
					case "submit": {
						if (!args.opportunity_id)
							throw new Error("opportunity_id is required for action='submit'");
						if (!args.base_template_id)
							throw new Error("base_template_id is required for action='submit'");
						if (!args.field_values)
							throw new Error("field_values is required for action='submit'");
									...
									text: JSON.stringify(
										{
											...
											status: result?.data?.completedAt
												? "submitted"
												: "draft",
										},
										null,
